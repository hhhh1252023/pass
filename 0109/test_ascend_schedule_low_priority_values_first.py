...
class TestLowPriorityFirstScheduling(CustomTestCase):
    ...
    def tearDownClass(cls):
        ...
        _verify_max_running_requests_and_max_queued_request_validation(1, 5)
        cls.stdout.close()
        cls.stderr.close()
    ...
    def test_low_priority_value_first_ordering(self):
        ...
        responses = asyncio.run(
            send_concurrent_generate_requests_with_custom_params(
                ...
                [
                    ...
                    # 优先级3 - 预期第四个完成
                    {"priority": 3, "sampling_params": {"max_new_tokens": 100}},
                ],
            )
        )
        ...
        assert e2e_latencies[0] < e2e_latencies[3] < e2e_latencies[2] < e2e_latencies[4] < e2e_latencies[1]
        ...
        responses = asyncio.run(
            send_concurrent_generate_requests_with_custom_params(
                ...
                [
                    ...
                    {"priority": 0, "sampling_params": {"max_new_tokens": 10000}},
                    # 优先级1 - 排队，不会被中止
                    {"priority": 1, "sampling_params": {"max_new_tokens": 100}},
                    # 优先级2 - 排队，不会被中止
                    {"priority": 2, "sampling_params": {"max_new_tokens": 100}},
