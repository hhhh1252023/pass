...
def run_unittest_files(
        ...
        while attempt <= (max_attempts if enable_retry else 1):
            ...
            try:
                ...
                if ret_code == 0:
                    ...
                    # 仅新增：收集失败用例名称
                    failed_test_names.append(filename)
                    break
    ...
    # 仅新增：打印失败用例名称汇总（无原因）
    logger.info(f"\n{'='*60}")
    logger.info(f"Failed Tests List (names only):")
    logger.info(f"{'='*60}")
    if failed_test_names:
        for idx, name in enumerate(failed_test_names, 1):
            logger.info(f"  {idx}. {name}")
    else:
        logger.info(f"  ✅ No failed tests")
    logger.info(f"{'='*60}\n")
