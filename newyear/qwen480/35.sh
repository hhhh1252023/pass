...
# pd传输, IP设置为p节点首节点
export ASCEND_MF_STORE_URL="tcp://172.22.3.71:24570"

# p节点IP
P_IP=('172.22.3.71')
# D节点IP D节点首节点IP
D_IP=('172.22.3.166' '172.22.3.188')
...
do
    ...
    then
        ...
        --chunked-prefill-size 20480 --max-prefill-tokens 20480 --moe-a2a-backend ascend_fuseep \
        --ep-dispatch-algorithm static --init-expert-location /home/weights/chenxu/hot_map/480_3.5k_prefill.pt \
...
do
    ...
    export GLOO_SOCKET_IFNAME=data0.3001
     ...
     export SGLANG_NPU_PROFILING_BS=136
    export SGLANG_NPU_FUSED_MOE_MODE=2
	...
	python -m sglang.launch_server --model-path ${MODEL_PATH} --disaggregation-mode decode --host ${D_IP[$i]} \
        ...
        --ep-dispatch-algorithm static --init-expert-location /home/weights/hot_map/480_3.5k_decode.pt \
        --cuda-graph-bs 16 32 56 72 80 88 96 104 112 120 128 136 --disaggregation-transfer-backend ascend --watchdog-timeout 9000 --context-length 8192 \
